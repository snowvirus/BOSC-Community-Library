---
title: Digital Literacy
last_updated: 2026-05-08
status: Verified
maintainer: Bazzenkya Francis
---

# Digital Literacy
Digital literacy resources help learners use computers, mobile devices, files, and the internet responsibly.

## Included Resources

### Digital Safety Basics

- Provider: Community ICT Desk
- License: CC BY 4.0
- Audience: Secondary learners
- Connectivity: Low bandwidth
- Summary: Introductory guidance on passwords, privacy, safe browsing, and responsible communication.

### Keyboard And File Management

- Provider: Community ICT Desk
- License: CC BY 4.0
- Audience: Beginner computer users
- Connectivity: Offline friendly
- Summary: Practice topics for keyboard use, folders, filenames, and simple document organization.
- Access Note: Offline-friendly practice guidance is maintained in this catalogue entry; no external download is required.

### Online Safety for Students

- Provider: Global Open Education
- License: CC BY 4.0
- Audience: All learners
- Connectivity: Low bandwidth
- Summary: A guide to staying safe online.
- Resource Link: [View Online Safety Module](https://www.commonsense.org/education/digital-citizenship)

## Link Validation Notes

- Checked on 2026-05-11: removed the broken placeholder PDF link for Keyboard And File Management.
- Checked on 2026-05-11: the Online Safety for Students module returned HTTP 200.

## Maintenance Notes

Future improvements should add link checks, practical worksheets, and local examples for schools with shared computer labs.
